...
```ts
function defineComponent<T extends ComponentTypes>(
...
type ComponentTypes = {
    ...
    nodes?: Record<string, Text | Element | Element[] | null>;
...
If you are using TypeScript, the generic argument is not required, but is highly recommended. By defining specific types for each feature of the component you include will add type safety and rich autocomplete to the component.
...
const FrozenYogurt = defineComponent({
    ...
    render: {
        message() {
            return `My favorite flavor is: ${this.flavor}`;
        },
    },
});
```
...
The component constructor is generated by the [defineComponent](#define-component) function. It is used to generate new instances for that component.
...
-   **`...state`**: properties defined in `state`
-   **`...nodes`**: properties defined in `nodes`
-   **`...data`**: properties defined in `data`

All functional component options (e.g. data, render, events, hooks) are bound to this object. This means that referencing the `this` keyword from within them will give you access to the data above.

Since the properties from the `state`, `nodes`, and `data` options are spread on this object, their keys must be unique or they will be overridden by one another.

State properties are reactive, updating the value of a state property assigned to `this` will trigger a component update.
...
This optional property defines the name of the component and is used primarily for error messaging.
...
-   **`query`**: Retrieves an element from the DOM with the following options:
    ...
    -   `scope` (optional): Declares the target of the query. By default, the root element is used.
...
Declare and store miscellaneous data on the component.
...
The `data` option takes an object with each each key representing the name of the property.

Any functions defined on this object will be bound automatically to the [internal component instance](#this).
...
-   **`root`**: Declares the root for the component. Must be a valid element or a CSS query selector.
...
const FrozenYogurt = defineComponent({
    ...
    components: {
        topping: {
            constructor: Cone,
            root: this.$root.querySelector('.cone'),
            state: {
                cone: this.coneType,
            },
            subscribe: {
...
interface ComponentOptions<T extends ComponentNodes> {
    events?: {
        ...
        [key: keyof ComponentTypes['nodes']]: (
            this: ComponentThis,
            index?: number
        ) => EventOption;
    };
